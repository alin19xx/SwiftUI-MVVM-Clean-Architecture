...
struct CryptoListUseCaseTests {
    ...
    func testFetchCryptoListingsSuccess() async throws {
        /// GIVEN
        var mockRepository = CryptoListRepositorySuccessMock()
        var useCase = CryptoListUseCase(repository: mockRepository)
        ...
        #expect(result.first?.name == "Ethereum")
    }
...
//            // WHEN - THEN
//            do {
//                _ = try await mockRepository.fetchCryptoInfo(for: ids)
//                throw TestError("Expected an error to be thrown, but no error was thrown.")
//            } catch let error as CryptoRepositoryError {
//                switch error {
